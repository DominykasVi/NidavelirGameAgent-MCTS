...
class MCTS:
    ...
    def save_run_info(self, time):
        with open('Results\\raw\\Iterations_runs\\iterations_runs_1.txt', 'a') as f:
            f.write(f'ED_{self.max_iterations}_{self.c_value}:{time}\n')
    ...
    def run_simulation(self, game_state:GameState, mcts_player_index:int, special_case=None) -> GameState:
        ...
        #         if card.color == 'coin':
###############################################################################################################################################################
        new_state = root_node.game_state.get_next_state()
        new_node = Node(root_node.game_state, root_node.depth, constant=root_node.constant)
        new_node.parent = root_node
        root_node.children.append(new_node)
        root_node.update_state(new_state, new_node)
        root_node.run_game_simulation(new_node, mcts_player_index) 

        for _ in range(60):
            has_states_unexplored = False
            if has_states_unexplored == False:
                has_states_unexplored, root_node.child_bets = root_node.has_next(root_node.child_bets)
            if has_states_unexplored == False:
                has_states_unexplored, root_node.child_takes = root_node.has_next(root_node.child_takes)

            if has_states_unexplored:
                new_node = root_node.generate_child_state()
                root_node.run_game_simulation(new_node, mcts_player_index)
        ...
        children = manager.list([None] * len(root_node.children))
        for child_node in root_node.children:
            index = root_node.children.index(child_node)
            process = Process(target=self.run_paralel_simulation, args=(root_node, child_node, mcts_player_index, children, index))
            processes.append(process)
            process.start()
            # self.run_paralel_simulation(root_node, child_node)
