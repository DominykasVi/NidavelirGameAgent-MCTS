...
class Node:
    def __init__(self, game_state:GameState, depth:int, name:str='', parent=None, constant=None, max_child_nodes:int=10) -> None:
        self.score = 0
        self.iterations = 0
        self.parent = parent
        self.children = []
    ...
    def find_max_child_node(self, total_runs:int) -> int:
        ...
        if self.max_child_nodes < len(self.children) or (self.game_state.turn in (6, 7, 8)):
        ...
        if additional_cards is not None:
            for bet in new_game_state.players[player_index].coins:
                if bet.exchangeable == False:
                    if coins_taken != 0:
                        new_node = Node(None, root_node.depth, constant=self.constant, max_child_nodes=root_node.max_child_nodes)
    ...
    def generate_child_state(self):
        ...
        if bets_left == True:
            new_node = Node(None, self.depth, constant=self.constant, max_child_nodes=self.max_child_nodes)
        ...
        if slots_left == True:
            new_node = Node(None, self.depth, self.constant, max_child_nodes=self.max_child_nodes)
            new_node.parent = self
            self.children.append(new_node)
            new_game_state = self.game_state.copy_state()
    ...
    def simulate_run(self, total_runs:int, mcts_player_index:int):
        ...
        if len(self.children) == 0:
            ...
            if new_state is None:
                ...
                new_node = Node(self.game_state, self.depth, constant=self.constant, max_child_nodes=self.max_child_nodes)
                new_node.parent = self
                self.children.append(new_node)
                #simulation
...
class MCTS:
    ...
    def handle_special_case(self, root_node:Node, special_case, mcts_player_index):
        if special_case[0] == 'distinction_selection':
            ...
            for card in root_node.child_takes:
                ...
                new_node = Node(root_node.game_state, root_node.depth, constant=root_node.constant, max_child_nodes=root_node.max_child_nodes)
                new_node.parent = root_node
                root_node.children.append(new_node)
    ...
    def run_simulation(self, game_state:GameState, mcts_player_index:int, special_case=None) -> GameState:
        ...
        self.c_value = c_val
        max_chld_nodes = game_state.players[mcts_player_index].max_child_nodes
        ...
        root_node = Node(game_state, 0, 'Root', constant=c_val, max_child_nodes=max_chld_nodes)
