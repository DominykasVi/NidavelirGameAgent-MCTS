...
class Game:
    ...
    def __init__(self, game_state: GameState, initialize=False) -> None:
        ...
        self.number_of_slot_cards = GameState.get_number_of_cards(
                ...
                while self.turn < self.age_split*2:
                    ...
                    try:
                        while self.slot_index < 4:
                            ...
                            self.print_function(
                                f"Player turns: {player_queue}")
                            GameState.exchange_crystals(
                                self.players, bet_index, self.mode)
                            self.print_function(
                                f"Crystals changed: {player_queue}")

                            try:
                                for player in player_queue:
                                    self.print_function(str(player))
                                    if player.card_taken == False:
                                        taken_card = self.take_card(
                                            player, bet_index)
                                        if player.has_row() and len(self.card_deck.hero_cards) != 0:
                                            ...
                                            self.print_function(
                                                f'Taken card: {hero}')
                                            action = GameState.hero_has_action(
                                                hero)
                                            if action == 'Discard':
                                                parameters = GameState.get_discard_parameters(
                                                    hero)
                                                self.print_function(
                                                    f'Hero {hero.name} discards cards')
                                                # here finished add hero action parser
                                                discarded_cards = player.discard_cards(
                                                    discard_card_count=parameters['discard_count'], available_colors=parameters['available_colors'])
                                                self.print_function(
                                                    f'Hero discards cards {str(discarded_cards)}')
                                            elif action == 'AddCoins':
                                                player.increase_coin(value=7)
                                        self.coin_test()
                                        self.print_function(
                                            f"Taken card {taken_card}")
                                        self.breakpoint()
                                    self.print_function(
    ...
    def coin_test(self):
        ...
        for coin_value in all_coins.keys():
            if all_coins[coin_value] > coin_limits[coin_value]:
                raise (Exception(
                    f'Illegal value {coin_value}, should be {coin_limits[coin_value]}, is {all_coins[coin_value]}'))
            ...
            if main == True:
                ...
                for player in self.players:
                    ...
                    for color in colors:
                        ...
                        ret_text.append(
                            f"Has {player.card_deck.card_count[color]} of {color}")
                        ret_text.append(f"Has {color_count} ranks of {color}")

                        card_sum += color_count
                    ret_text.append(
                        f"Has {str(player.card_deck.heroes)} heroes")
                    ret_text.append(
                        f"Has {str(len(player.card_deck.hero_cards))} hero cards")
                ...
                for player in self.players:
                    ...
                    self.print_function(
                        player.card_deck.get_card_deck_string())
                    card_sum = 0
                    for color in colors:
                        color_count = player.get_color_count(color)
                        self.print_function(
                            f"Has {player.card_deck.card_count[color]} of {color}")
                        self.print_function(
                            f"Has {color_count} ranks of {color}")
